...
export default function EditProductPage() {
  ...
  const [allConcerns, setAllConcerns] = useState<{ id: string; title: string }[]>([]);
  const [allPersonas, setAllPersonas] = useState<{ id: string; title: string }[]>([]);
  ...
  async function load() {
    ...
    const mappingsRes = await fetch(`/api/admin/products/${id}/mappings`, {
  cache: "no-store",
    });
    const mappingsData = await mappingsRes.json().catch(() => null);

    if (mappingsRes.ok && mappingsData) {
      setAllConcerns(mappingsData.concerns ?? []);
      setAllPersonas(mappingsData.personas ?? []);
      setSelectedConcernIds(mappingsData.selectedConcernIds ?? []);
      setSelectedPersonaIds(mappingsData.selectedPersonaIds ?? []);
    }
  }
  ...
  async function saveBase() {
    ...
    await fetch(`/api/admin/products/${id}/mappings`, {
  method: "PUT",
  headers: { "Content-Type": "application/json" },
  body: JSON.stringify({
  ...
  <div>
    ...
    <p className="mt-1 text-sm text-black/70">
      Select which concerns this product should appear under.
    </p>
      ...
      {allConcerns.map((concern) => (
        <label key={concern.id} className="flex items-center gap-2 text-sm">
          <input
            ...
            onChange={(e) => {
              if (e.target.checked) {
                setSelectedConcernIds((prev) => [...prev, concern.id]);
              } else {
  ...
  <div>
    ...
    <p className="mt-1 text-sm text-black/70">
      Select which personas this product should appear under.
    </p>
