...
export async function GET() {
  try {
    ...
    if (!sectionId) {
      return NextResponse.json({ error: "Home section not found: cure_your_concerns" }, { status: 404 });
    }

    const [allRes, selectedRes] = await Promise.all([
      pool.query(`select * from concerns where is_active = true order by title asc`),
      pool.query(
    ...
    for (const it of items) {
      ...
      const upd = await pool.query(
        `update home_section_items
         ...
         where section_id = $2::uuid
           and item_type = $3
           and ref_id = $4::uuid`,
        [display_order, sectionId, "concern", ref_id]
      );
      ...
      if (upd.rowCount === 0) {
        await pool.query(
          ...
          [sectionId, "concern", ref_id, display_order]
        );
      }
    }
