...
export async function GET() {
  try {
    ...
    return NextResponse.json(
      { error: "Failed to load concerns", detail: err?.message ?? String(err) },
      { status: 500 }
    );
  }
}
...
export async function POST(req: Request) {
  try {
    ...
    const res = await pool.query(
      `insert into concerns (title, image_url, filter_key, is_active)
       values ($1,$2,$3,$4)
    ...
    return NextResponse.json(
      { error: "Failed to create concern", detail: err?.message ?? String(err) },
      { status: 500 }
    );
  }
}